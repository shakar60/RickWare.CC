...
function SmoothDragging(DragCfg)
    DragCfg = DragCfg or {}
    DragCfg.Smoothness = DragCfg.Smoothness
    DragCfg.ClampToScreen = DragCfg.ClampToScreen
    DragCfg.TweenStyle = DragCfg.TweenStyle
    DragCfg.TweenDirection = DragCfg.TweenDirection

    local function clampPosition(position, guiObject)
        if not DragCfg.ClampToScreen then return position end
        ...
        local absoluteX = position.X.Scale * screenSize.X + position.X.Offset
        local absoluteY = position.Y.Scale * screenSize.Y + position.Y.Offset
        
        local clampedX = math.clamp(absoluteX, 0, screenSize.X - objectSize.X)
        local clampedY = math.clamp(absoluteY, 0, screenSize.Y - objectSize.Y)
        
        return UDim2.new(
            position.X.Scale,
            clampedX - position.X.Scale * screenSize.X,
            position.Y.Scale,
            clampedY - position.Y.Scale * screenSize.Y
        )
    end
    ...
    function SmoothDrag.new(dragObject, targetObject, smoothEnabled, callbackOnly)
        local self = setmetatable({}, SmoothDrag)
        self.Object = dragObject
        self.ToMove = targetObject
        self.Smooth = smoothEnabled
    ...
    function SmoothDrag:Enable()
        self.CanBeDragged = true
        local dragObject = self.Object
        local targetObject = self.ToMove
        ...
        local function updatePosition(input)
            ...
            if not self.CallbackOnly then
                newPosition = clampPosition(newPosition, dragObject)

                if (self.Smooth == nil or self.Smooth == true) and self.Smooth ~= false then
                    TweenService:Create(
                        targetObject or dragObject,
                        TweenInfo.new(DragCfg.Smoothness, DragCfg.TweenStyle, DragCfg.TweenDirection),
                        { Position = newPosition }
                    ):Play()
                else
                    local obj = targetObject or dragObject
        ...
        self.InputBegan = dragObject.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                connection = input.Changed:Connect(function()
                    if input.UserInputState == Enum.UserInputState.End and (self.Dragging or isInputDown) then
                        self.Dragging = false
                        connection:Disconnect()
                        if self.DragEnded and not isInputDown then
        ...
        self.InputChanged = dragObject.InputChanged:Connect(function(input)
        ...
        self.InputChanged2 = UserInputService.InputChanged:Connect(function(input)
            ...
            if isInputDown then
                ...
                originalPosition = targetObject and targetObject.Position or dragObject.Position
...
local SmoothDrag = SmoothDragging(DragCfg)
...
do
    ...
    Library:SmoothDragging(Library.Watermark);
    ...
    Library:SmoothDragging(KeybindOuter);
end;
...
function Library:CreateWindow(...)
    ...
    Library:SmoothDragging(Outer, 25);
