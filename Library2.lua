...
do
    ...
    function ObsidianImageManager.GetAsset(AssetName: string)
        ...
        if getcustomasset then
            local success, result = pcall(function()
                return getcustomasset(AssetData.Path)
            end)
    ...
    function ObsidianImageManager.DownloadAsset(AssetPath: string)
        if not (getcustomasset and writefile and isfile and syn) then
            return
        end

        local URLPath = AssetPath:gsub("Obsidian/", "")
        local content = game:HttpGet(`{BaseURL}{URLPath}`)
        ...
        if not success then
            warn("Failed to download asset:", AssetPath, "Error:", err)
        end
    end

    for _, Data in pairs(ObsidianImageManager.Assets) do
        pcall(ObsidianImageManager.DownloadAsset, Data.Path)
    end
end

return ObsidianImageManager
