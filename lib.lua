...
function SmoothDrag.new(dragObject, targetObject, smoothEnabled, callbackOnly)
    ...
    self.TargetObject = targetObject or dragObject
    self.Smooth = smoothEnabled == nil and true or smoothEnabled
...
function SmoothDrag:Enable()
    ...
    local function updatePosition(input)
        ...
        if not self.CallbackOnly then
            local screenGui = dragObject:FindFirstAncestorWhichIsA("ScreenGui")
            if screenGui then
                local screenSize = screenGui.AbsoluteSize
                local objectSize = targetObject.AbsoluteSize
                
                local absoluteX = newPosition.X.Scale * screenSize.X + newPosition.X.Offset
                local absoluteY = newPosition.Y.Scale * screenSize.Y + newPosition.Y.Offset
                ...
                newPosition = UDim2.new(
                    newPosition.X.Scale,
                    clampedX - newPosition.X.Scale * screenSize.X,
                    newPosition.Y.Scale,
                    clampedY - newPosition.Y.Scale * screenSize.Y
                )
            end
        end
        ...
        if self.Smooth then
            ...
            TweenService:Create(targetObject, tweenInfo, {Position = newPosition}):Play()
        else
    ...
    self.Connections.inputChanged = dragObject.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            inputObject = input
        end
    end)
    
    self.Connections.inputChanged2 = UserInputService.InputChanged:Connect(function(input)
...
do
    ...
    local drag = SmoothDrag.new(Watermark, Watermark, true)
    drag:Enable()
    ...
    local drag = SmoothDrag.new(KeybindOuter, KeybindOuter, true)
    drag:Enable()
end;
...
function Library:CreateWindow(...)
    ...
    local drag = SmoothDrag.new(Outer, Outer, true)
    drag:Enable()
